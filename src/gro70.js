...
function displayRecords(records) {
    ...
    records.forEach(record => {
        ...
        const targetCoords = groUtils.getCoordsInTargetMode(record.fields, currentCoordMode);
        const formattedX = groUtils.formatCoordinate(targetCoords.x);
        const formattedY = groUtils.formatCoordinate(targetCoords.y);
        const formattedH = groUtils.formatCoordinate(h);
        ...
        if (typeof proj4 !== 'undefined') {
            const mskCoords = groUtils.getCoordsInTargetMode(record.fields, 'msk');
...
function findAndShowNearby(clickedRecordId) {
    ...
    const refCoords = groUtils.getCoordsInTargetMode(referencePoint.fields, currentCoordMode);
    ...
    fullDatabaseCache.forEach(candidateRecord => {
        ...
        const candCoords = groUtils.getCoordsInTargetMode(candidateRecord.fields, currentCoordMode);
    ...
    let refOutputString = `${refPointData['Point'] || 'N/A'}${currentSeparatorChar}${groUtils.formatCoordinate(refX)}${currentSeparatorChar}${groUtils.formatCoordinate(refY)}${currentSeparatorChar}${groUtils.formatCoordinate(refPointData['H'])}${currentSeparatorChar}`;
...
function findNearbyUserLocation(latitude, longitude) {
    ...
    try {
        ...
        userCoordsInCurrentMode = groUtils.transformCoordinates(userMsk_x, userMsk_y, 'msk', currentCoordMode.toLowerCase());
    } catch (e) {
    ...
    userLocationHeader.innerHTML = `<strong>Ваше местоположение (в СК ${currentCoordMode.toUpperCase()}):</strong> X: ${groUtils.formatCoordinate(userRefX)}, Y: ${groUtils.formatCoordinate(userRefY)}`;
...
function populateNearbyList(nearbyPointsArray) {
    if (nearbyPointsArray.length > 0) {
        nearbyPointsArray.forEach(item => {
            ...
            let outStr = `${rd['Point'] || 'N/A'}${currentSeparatorChar}${groUtils.formatCoordinate(item.calculatedX)}${currentSeparatorChar}${groUtils.formatCoordinate(item.calculatedY)}${currentSeparatorChar}${groUtils.formatCoordinate(rd['H'])}`;
            ...
            contentSpan.innerHTML = `<span class="nearby-distance">(${item.distance.toFixed(1)}м)</span> ${rd['Point'] || 'N/A'}${currentSeparatorChar}${groUtils.formatCoordinate(item.calculatedX)}${currentSeparatorChar}${groUtils.formatCoordinate(item.calculatedY)}`;
            li.appendChild(contentSpan);


            if (typeof proj4 !== 'undefined') {
                const pointMskCoords = groUtils.getCoordsInTargetMode(rd, 'msk');
