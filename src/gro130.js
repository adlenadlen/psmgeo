		...
		function displayRecords(records) {
			...
			records.forEach(record => {
				...
				const formattedX = groUtils.formatCoordinate(calculatedX);
				const formattedY = groUtils.formatCoordinate(calculatedY);
		...
		function findAndShowNearby(clickedRecordId) {
			...
			const refFormattedX = groUtils.formatCoordinate(refX);
			const refFormattedY = groUtils.formatCoordinate(refY);
			const refFormattedH = groUtils.formatCoordinate(refPointData['H']);
			...
			if (nearbyPoints.length > 0) {
				nearbyPoints.forEach(item => {
					...
					const formattedX = groUtils.formatCoordinate(item.calculatedX); // Используем уже рассчитанные
					const formattedY = groUtils.formatCoordinate(item.calculatedY); // Используем уже рассчитанные
					const formattedH = groUtils.formatCoordinate(recordData['H']);
		...
		async function fetchAllRecords() {
			...
			try {
				...
				fullDatabaseCache = groUtils.parseCSV(csvText);
				isLoading = false;
		...
		searchInput.addEventListener('input', () => {
			...
			debounceTimer = setTimeout(groUtils.performSearch, DEBOUNCE_DELAY);
		});
		searchModeToggleButtons.forEach(radio => {
			radio.addEventListener('change', () => {
				if (searchInput.value.trim() !== '') {
					groUtils.performSearch();
				}
			});
		});
